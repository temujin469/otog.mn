...
export default function PostCard({ blog }: Props) {
  ...
  return (
    <Link to={`/blogs/${blog.id}`} className="w-full mb-5">
      ...
      <p className="text-[13px] mt-2 text-gray flex items-center gap-1"><BsCalendarDate />{moment(blog.date).format('ll')}</p>
      <h3 className="heading-sm m-0">{blog.title.length > 64 ? `${blog.title.slice(0,64)}...` : blog.title}</h3>
